...
### Changed
- Переход на **MCP SDK 2.0**: `mcp.server.fastmcp.FastMCP` → `mcp.server.mcpserver.MCPServer`.
  Набор инструментов, их имена, аргументы и поведение не изменились — для пользователей
...
- Обновлены GitHub Actions: `checkout` v4→v7, `setup-uv` v5→v9.0.0, `upload-artifact` v4→v7,
...
- Тесты сборки сервера (`tests/test_server.py`): регистрация инструментов, пропуск
  ненастроенных модулей, фильтр `YANDEX360_ENABLE`, генерация схем аргументов. Такой тест
  ловит переезды API SDK до релиза.
- Шаг релизного пайплайна: собранное колесо ставится в чистое окружение и запускается —
  проверка того же пути, которым пакет ставится у пользователя через `uvx`.
- Аннотации инструментов (`readOnlyHint` / `destructiveHint` / `openWorldHint`). Клиент
  теперь видит, что чтение безопасно, а `wiki_delete_page` и `wiki_update_page` (content
  заменяет тело страницы целиком) — разрушающие, и может спросить подтверждение.
